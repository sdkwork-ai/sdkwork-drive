...
pub async fn bootstrap_drive_database(pool: DatabasePool) -> Result<DriveDatabaseHost, String> {
    ...
    // (DATABASE_FRAMEWORK_SPEC §4.2); repair runs `db:migrate`. The drive module
    // is composed same-origin by the standalone gateway, so a drift here would
    ...
    let drift = DriftEngine::new(pool.clone(), module.clone())
        .analyze()
        .await
        .map_err(|error| format!("drive database drift check failed: {error}"))?;
    if drift.summary.error > 0 {
        let details = drift
            ...
            .join("; ");
        return Err(format!(
            "drive database schema drift detected ({} error(s)): {details}. Run `pnpm db:migrate` and then `pnpm db:drift:check`",
            drift.summary.error
        ));
    }
