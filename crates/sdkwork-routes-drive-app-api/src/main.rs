...
async fn main() {
    ...
    sdkwork_drive_observability::init_tracing("sdkwork-routes-drive-app-api");
    ...
    let bind_addr = bind_addr_from_env("SDKWORK_DRIVE_APP_API_BIND", "127.0.0.1:18080");
    serve_router(router, bind_addr, "sdkwork-routes-drive-app-api")
        .await
        .expect("serve app api");
}
