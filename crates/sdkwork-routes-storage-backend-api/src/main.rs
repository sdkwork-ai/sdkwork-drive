...
use sdkwork_routes_storage_backend_api::build_router_with_database_config;

#[tokio::main]
async fn main() {
    sdkwork_drive_workspace_service::enable_process_shared_database_pool();
    sdkwork_drive_observability::init_tracing("sdkwork-routes-storage-backend-api");
    ...
    serve_router(router, bind_addr, "sdkwork-routes-storage-backend-api")
        .await
